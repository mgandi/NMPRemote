...
{
    int index = 0;
    //    const quint8* data = section.constData();
    UInt32 tmp = 0xffffffff;
    
    while (index < [section.data length]) {
        UInt32 a = tmp << 8;
        UInt32 b = tmp >> 24;
        tmp = a ^ crc32Table[b] ^ ((const char *)[section.data bytes])[index++];
        //        tmp = (tmp << 8) ^ crc32Table[(tmp >> 24) ^ section[index]];
    }
...
- (void)tableComplete
{
    [_pidHandlerDelegate parseTable:(ALiSection *)section];
}

- (void)parse:(NSData *)packet
{
    // Make sure there are data in this packet
    if (![packet length])
        return;
    
    // Compute pointer to data start
    const unsigned char *data = (const unsigned char *)[packet bytes], *p = 0;
    
    // Check that data starts with 0x47
    if (data[0] != 0x47) {
        NSLog(@"Not a TS packet!");
        return;
    }
    
    // Check validity of continuity counter
    UInt8 ccc = data[3] & 0x0F;
    ...
    if (!section) {
        ...
        section = [[ALiSection alloc] init];
    }
    
    
    
    // Compute availability of data
    UInt8 available = 188 + data - p;
    
    while (available) {
        // Check if there is enough bytes in the current data buffer to complete the need of the section
        if (available >= section.need) {
            
            // Add the needed bytes to the data buffer of the section
            [section.data appendData:[NSData dataWithBytes:(const void *)p length:section.need]];
            p += section.need;
            available -= section.need;
            
            // Check if the section's header has been parsed so far
            if (!section.headerComplete) {
                // Apparently not! So let's do the job
                section.need = section.sectionLength = (((UInt16)((const char *)[section.data bytes])[1] & 0x0F) << 8) | ((UInt16)((const char *)[section.data bytes])[2] & 0xFF);
                
                // Done! Let's indicate it
                section.headerComplete = true;
                
                // We still need to check if the section isn't too long
                if (section.need > _sectionMaxSize - 3) {
//                    NSLog(@"Section too long!");
                ...
                if (_hasCRC) {
                    section.validCRC = [self checkCRC];
                }
                
                section.tableID = ((const char *)[section.data bytes])[0];
                section.sectionSyntaxIndicator = (((const char *)[section.data bytes])[1] & 0x80) >> 7;
                section.serviceID = (((UInt16)((const char *)[section.data bytes])[3] & 0xFF) << 8) | ((UInt16)((const char *)[section.data bytes])[4] & 0xFF);
                section.versionNumber = (((const char *)[section.data bytes])[5] & 0x3E) >> 1;
                section.currentNextIndicator = ((const char *)[section.data bytes])[5] & 0x01;
                section.sectionNumber = ((const char *)[section.data bytes])[6];
                section.lastSectionNumber = ((const char *)[section.data bytes])[7];
                
                section.payload = (const UInt8 *)&((const char *)[section.data bytes])[8];
                section.payloadSize = [section.data length] - 8;
                [self tableComplete];
            ...
            // There aren't enough bytes in this packet to complete the section.
            // So append the available bytes to the data buffer of the section
            [section.data appendData:[NSData dataWithBytes:(const void *)p length:available]];
            section.need -= available;
            available = 0;
        }
    }
}
...
- (id)init
{
    // Init section max size;
    _sectionMaxSize = 1024;
